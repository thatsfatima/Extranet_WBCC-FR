...
class Opportunity extends Model
{
    ...
    public function getNbTacheEffectGest($user, $periode, $date1, $date2, $code, $statut, $typeIntervention)
    {
        ...
        if ($code == 0) {
            ...
            $columnDate = "o.$activity->nomVariableDateOP";
    ...
    public function getListeOPByTypeAuteurPeriode($periode, $columnDate, $columnType, $date1, $date2, $user, $columnAut, $code)
    {
        if ($code == 0) {
            $sql = "SELECT * FROM wbcc_opportunity, wbcc_rendez_vous r WHERE idOpportunity = r.idOpportunityF AND r.etatFRT = 0 ";
            $columnDate = "r.$columnDate";
            $columnAut = "r.$columnAut";
        } else {
            $sql = "SELECT * FROM wbcc_opportunity WHERE $columnType = 1 AND etatOP = 1";
        }
    ...
    public function getOPCCTP($idCompany, $etapeCCTP)
    {
        $req = "";
        //FILTRER PAR ETAPE
        if ($etapeCCTP == "creation") {
            $req .= " AND d.sendMailDD=1 ";
        } else {
            if ($etapeCCTP == "aValider") {
                $req .= " AND d.aValider=1";
            }
        }
        $this->db->query("SELECT * FROM wbcc_opportunity as op, wbcc_opportunity_devis as od, wbcc_devis as d WHERE op.idOpportunity=od.idOpportunityF AND od.idDevisF=d.idDevis AND od.valide=1 AND d.typeDevis='CCTP' AND d.idSocieteArchi=$idCompany $req");
